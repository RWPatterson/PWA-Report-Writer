/* =====================================================================================
   iso19438DisplaySizesView.js
   =====================================================================================
   Own copy of iso454812DisplaySizesView.js's persistence + dialog — same job (persist
   a "always show these sizes" preference per sensor; let a user pick up to 16 of the
   currently-loaded file's measured sizes), not shared, per CLAUDE.md. See that file's
   own header comment for the full rationale (per-sensor scoping, why a dynamic dialog
   not a static template) — not repeated here.
   ===================================================================================== */
import { loadCustomDefaults, saveCustomDefaults } from "../../report/customDefaults.js";

//#region persistence
/** @param {string} sensor @returns {string} reserved key inside the per-standard customDefaults blob */
function displaySizesKey(sensor) {
  return "__iso19438DisplaySizes__" + sensor;
}

/** @param {string} standardId @param {string} sensor @returns {number[]|null} */
...
export function openDisplaySizesDialog({ availableSizes, currentSizes, onSave }) {
  ...
  const dialog = el("div", { class: "modal-box" },
    ...
    el("p", { class: "hint" }, "Up to " + MAX_SIZES + " sizes fill Page 1's Filtration Efficiencies table and the per-window " +
      "clump pages. This choice is saved as a default for future ISO 19438 reports using THIS sensor only — LB and LS keep " +
      "separate saved lists. A size not measured by some later file falls back to that file's own configured sizes for just that slot."),
    sizeList,
    countLine,
    errorLine,
