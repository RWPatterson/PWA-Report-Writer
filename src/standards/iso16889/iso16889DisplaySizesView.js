/* =====================================================================================
   iso16889DisplaySizesView.js
   =====================================================================================
   Own copy of iso454812DisplaySizesView.js's shape, per CLAUDE.md — same two jobs:

     loadPersistedDisplaySizes / savePersistedDisplaySizes — persist a user's "always
       show these sizes" preference, scoped PER SENSOR (LB and LS have different
       measured size ranges), using the same reserved-key-inside-customDefaults
       mechanism every clean standard's DisplaySizesView uses.
     ...
     openDisplaySizesDialog — the modal for picking up to 16 of the CURRENTLY loaded
       ...
       whatever's currently resolved. Built dynamically, not a static template,
       because which sizes are even choosable depends on the loaded file and sensor.

   16 sizes matches Page 2's own table width (16 size columns, "d>"/"β" pairs) — not
   a coincidence with 4548-12's own MAX_SIZES, just the same practical ceiling.
   ===================================================================================== */
...
function displaySizesKey(sensor) {
  return "__iso16889DisplaySizes__" + sensor;
}
...
export function openDisplaySizesDialog({ availableSizes, currentSizes, onSave }) {
  ...
  const dialog = el("div", { class: "modal-box" },
    ...
    el("p", { class: "hint" }, "Up to " + MAX_SIZES + " sizes fill Page 1's filtration ratio table and Page 2's particle-count table. This choice is saved as a default for future " +
      "ISO 16889 reports using THIS sensor only — LB and LS keep separate saved lists. A size not measured by some later file falls back to that file's own configured sizes for just that slot."),
    sizeList,
    countLine,
    errorLine,
